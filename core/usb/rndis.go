...
// DNS 通过 resolvconf 的 tail 文件更新:resolvconf 生成 /etc/resolv.conf
// 时把该文件内容追加到末尾 —— NetworkManager 走 resolvconf 后端重写
// resolv.conf 时条目不丢,不像直接写文件会被覆盖。写入后 resolvconf -u
// 立即重新聚合。tail 原有内容先备份,离开从模式时还原。
const (
	resolvConfTail   = "/etc/resolvconf/resolv.conf.d/tail"
	resolvConfBackup = "/tmp/rndis-resolvconf-tail"
)

// applyDns 把上游 DNS(探测拿到的,没有则用上游网关兜底)写进 resolvconf
// tail 文件。
func (this *UsbGadgetRndis) applyDns(dns []netip.Addr, fallback netip.Addr) {
	if len(dns) == 0 {
		dns = []netip.Addr{fallback}
	}
	backupResolvConfTail()
	...
	if err := os.MkdirAll(filepath.Dir(resolvConfTail), 0755); err != nil {
		log.Printf("WARN: mkdir %s: %v\n", filepath.Dir(resolvConfTail), err)
		return
	}
	if err := os.WriteFile(resolvConfTail, []byte(sb.String()), 0644); err != nil {
...
// backupResolvConfTail 备份 tail 原内容(首次进入从模式时;上次的备份
// 还没恢复说明仍在从模式,不重复备份)。
func backupResolvConfTail() {
	if _, err := os.Stat(resolvConfBackup); err == nil {
		return
	}
	data, err := os.ReadFile(resolvConfTail)
	if err != nil {
		return // tail 原本不存在,restore 时按空还原
	}
	_ = os.WriteFile(resolvConfBackup, data, 0644)
}

// restoreDns 离开从模式(回退/切回主模式)时还原 tail 原状。
func restoreDns() {
	if data, err := os.ReadFile(resolvConfBackup); err == nil {
		_ = os.WriteFile(resolvConfTail, data, 0644)
		_ = os.Remove(resolvConfBackup)
	} else {
		_ = os.Remove(resolvConfTail) // 无备份:tail 原本不存在
	}
	reloadResolvconf()
}

// reloadResolvconf 重新聚合 resolv.conf,使 tail 改动立即生效。
func reloadResolvconf() {
	if out, err := exec.Command("resolvconf", "-u").CombinedOutput(); err != nil {
		log.Printf("WARN: resolvconf -u: %v, output: %s\n", err, string(out))
	}
}
